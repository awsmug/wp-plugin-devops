...
# Start php-fpm.
tpl "$BIN_PATH/bin/php-fpm.tpl.conf" "$PHP_FPM_CONF"
"$PHP_FPM_BIN" --fpm-config "$PHP_FPM_CONF"

# Build the default nginx config files.
tpl "$BIN_PATH/bin/nginx.tpl.conf" "$NGINX_DIR/nginx/nginx.conf"
tpl "$BIN_PATH/bin/fastcgi.tpl.conf" "$NGINX_DIR/nginx/fastcgi.conf"
tpl "$BIN_PATH/bin/default-site.tpl.conf" "$NGINX_DIR/nginx/sites-enabled/default-site.conf"
