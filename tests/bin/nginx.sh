...
# Start php-fpm.
tpl "$WH_DIR/tests/bin/php-fpm.tpl.conf" "$PHP_FPM_CONF"
"$PHP_FPM_BIN" --fpm-config "$PHP_FPM_CONF"

# Build the default nginx config files.
tpl "$WH_DIR/tests/bin/nginx.tpl.conf" "$WH_NGINX_DIR/nginx/nginx.conf"
tpl "$WH_DIR/tests/bin/fastcgi.tpl.conf" "$WH_NGINX_DIR/nginx/fastcgi.conf"
tpl "$WH_DIR/tests/bin/default-site.tpl.conf" "$WH_NGINX_DIR/nginx/sites-enabled/default-site.conf"
